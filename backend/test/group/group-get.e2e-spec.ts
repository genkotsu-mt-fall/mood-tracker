...
import {
  GroupClient,
  SupertestGroupMembersResponse,
} from 'test/clients/group.client';
...
describe('GroupController', () => {
  ...
  describe('GroupController (GET /group/:id)', () => {
    it('should return a specific group with auth', async () => {
      ...
      const res: SupertestGroupResponse = await GroupClient.get(
      ...
      expect(res.body.data.name).toBe(group.name);
      expect(res.body.data.userId).toBe(groupOwner.profile.id);
    });
  ...
  describe('GroupController (GET /group/:id/members)', () => {
    ...
    it('should return a list of group members when the group exists and has members', async () => {
      ...
      const res: SupertestGroupMembersResponse = await GroupClient.findMembers(
        groupOwner.accessToken,
        group.id,
      );
      expect(res.status).toBe(200);
      ...
      expect(res.body.data[0]).toHaveProperty('id', groupMember.memberId);
    });
  ...
  describe('GroupController (GET /auth/me/groups)', () => {
    it('should return all groups created by the authenticated user', async () => {
      const { group, groupOwner } = await GroupUseCase.createGroup(prefix);
      const res: SupertestResponse<ApiResponse<Group[]>> =
        await GroupClient.getOwnedGroups(groupOwner.accessToken);
      expect(res.status).toBe(200);
    ...
    it('should return an empty array if the user has not created any groups', async () => {
      ...
      const res: SupertestResponse<ApiResponse<Group[]>> =
        await GroupClient.getOwnedGroups(user.accessToken);
      expect(res.status).toBe(200);
      expect(Array.isArray(res.body.data)).toBe(true);
      expect(res.body.data.length).toBe(0);
    });
