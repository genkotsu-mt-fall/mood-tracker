...
describe('FollowController (POST /follow)', () => {
  ...
  it('should fail to follow oneself', async () => {
    ...
    const res = await FollowClient.follow(user.accessToken, user.profile.id);
  ...
  it('should return 404 when followeeId is a valid UUID but does not exist in DB', async () => {
    ...
    const res = await FollowClient.follow(user.accessToken, nonExistentId);

    const body = res.body as { message: string };
    expect(res.status).toBe(404);
    expect(body.message).toContain(`User with id ${nonExistentId} not found`);
  });
});
